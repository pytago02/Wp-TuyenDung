<?php $Level = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $ewd_feup_levels_table_name WHERE Level_ID ='%d'", intval( $_GET['Level_ID'] ) ) ); ?>
<?php $Fields = $wpdb->get_results( "SELECT Field_Name, Field_ID, Level_Exclude_IDs FROM $ewd_feup_fields_table_name" ); ?>
<?php $Users = $wpdb->get_results( $wpdb->prepare( "SELECT User_ID, Username FROM $ewd_feup_user_table_name where Level_ID=%d", intval( $_GET['Level_ID'] ) ) ); ?>
