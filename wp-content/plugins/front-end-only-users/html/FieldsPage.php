...
<?php 
			...
			if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Field_Name', 'Field_Type', 'Field_Description', 'Field_Required' ) ) ) { $OrderBy = $_GET['OrderBy']; }
			else { $OrderBy = 'Field_Name'; }

			$Order = ( isset( $_GET['Order'] ) and $_GET['Order'] == 'DESC' ) ? 'DESC' : 'ASC';
			
			$Sql = "SELECT * FROM $ewd_feup_fields_table_name ";
				if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Fields") {$Sql .= "ORDER BY " . $OrderBy . " " . $Order . " ";}
		 ...
		 <?php
				if ($myrows) { 
	  			  foreach ($myrows as $Field) {
								...
								echo "<a class='delete-tag' href='" . add_query_arg( '_wp_nonce', wp_create_nonce( 'ewd-feup-admin' ), 'admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_DeleteField&DisplayPage=Fields&Field_ID=' . $Field->Field_ID ) . "'>" . __("Delete", 'front-end-only-users') . "</a>";
		 						echo "</span>";
								echo "</div>";
