	...
	<div class="ewd-feup-admin-new-product-page-top-part-right">
		...
		<div class="ewd-feup-admin-export-buttons-area">
			<?php if($EWD_FEUP_Full_Version == 'Yes'){ ?>
				<form method="post" action="admin.php?page=EWD-FEUP-options&DisplayPage=Users&Action=EWD_FEUP_ExportToExcel">
...
<?php
	...
	if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'User_Admin_Approved', 'User_WP_ID', 'User_Last_Login', 'User_Date_Created' ) ) or ewd_feup_field_name_exists( $_GET['OrderBy'] ) ) { $OrderBy = $_GET['OrderBy']; }
	else { $OrderBy = 'User_Date_Created'; }
	...
	if (isset($_REQUEST['UserSearchValue']) and $_REQUEST['UserSearchField'] == "Username") {
		$Sql .= "WHERE " . esc_sql( $_REQUEST['UserSearchField'] ) . " ";
		if ($_REQUEST['UserSearchOperator'] == "LIKE") {$Sql .= " LIKE '%". esc_sql( $_REQUEST['UserSearchValue'] ) . "%' ";}
		else {$Sql .= "='" . esc_sql( $_REQUEST['UserSearchValue'] ) . "' ";}
	}
	elseif (isset($_REQUEST['UserSearchValue'])) {
		$Sql .= "WHERE Field_ID=" . esc_sql( $_REQUEST['UserSearchField'] ) . " AND Field_Value ";
		if ($_REQUEST['UserSearchOperator'] == "LIKE") {$Sql .= " LIKE '%". esc_sql( $_REQUEST['UserSearchValue'] ) . "%' ";}
		else {$Sql .= "='" . esc_sql( $_REQUEST['UserSearchValue'] ) . "' ";}
	}
	if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Users") {
		if ($_REQUEST['OrderBy'] == "User_Last_Login" or $_REQUEST['OrderBy'] == "User_Date_Created" or $_REQUEST['OrderBy'] == "User_Admin_Approved") {$Sql .= "ORDER BY " . $OrderBy . " " . $Order . " ";}
		else {
			$OrderBy_Field_ID = $wpdb->get_var($wpdb->prepare("SELECT Field_ID FROM $ewd_feup_fields_table_name WHERE Field_Name=%s", $OrderBy));
			$User_IDs = $wpdb->get_results($wpdb->prepare("SELECT User_ID FROM $ewd_feup_user_fields_table_name WHERE Field_ID=%d ORDER BY Field_Value %s", $OrderBy_Field_ID, $Order));
			$OrderBy_User_IDs = array();
			foreach ($User_IDs as $User_ID) {$OrderBy_User_IDs[] = $User_ID->User_ID;}
			$Sql .= "ORDER BY FIELD($ewd_feup_user_table_name.User_ID, " . esc_sql( implode(",", $OrderBy_User_IDs) ) . ")";
		}
	}
	...
	if (isset($_REQUEST['UserSearchValue'])) {$Current_Page_With_Order_By .= "&UserSearchValue=" . rawurlencode( $_REQUEST['UserSearchValue'] ) . "&UserSearchField=" . rawurlencode( $_REQUEST['UserSearchField'] ) . "&UserSearchOperator=" . rawurlencode( $_REQUEST['UserSearchOperator'] );}
?>

<form action="<?php echo add_query_arg( '_wp_nonce', wp_create_nonce( 'ewd-feup-admin' ), 'admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_MassUserAction&DisplayPage=Users' ); ?>" method="post">
<p class="search-box">
	...
	<br />
	<a class='feup-confirm-all-users button-secondary action ewd-feup-admin-delete-all-products-button' href='<?php echo add_query_arg( '_wp_nonce', wp_create_nonce( 'ewd-feup-admin' ), 'admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_DeleteAllUsers&DisplayPage=Users' ); ?>'><?php _e("Delete All Users", 'front-end-only-users'); ?></a>
</p>
		...
		if ($myrows) {
	  		foreach ($myrows as $User_ID) {
				...
				foreach ($Fields as $Field) {
					...
					echo substr( strval($User_Field_Value), 0, 60 );
