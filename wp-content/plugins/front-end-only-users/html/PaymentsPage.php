...
<?php 
			...
			if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Username', 'Payment_Date', 'Payment_Amount', 'Next_Payment_Date', 'Discount_Code_Used' ) ) ) { $OrderBy = $_GET['OrderBy']; }
			else { $OrderBy = 'Username'; }

			$Order = ( isset( $_GET['Order'] ) and $_GET['Order'] == 'DESC' ) ? 'DESC' : 'ASC';
			
			$Sql = "SELECT * FROM $ewd_feup_payments_table_name ";
				if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Payments") {$Sql .= "ORDER BY " . $OrderBy . " " . $Order . " ";}
