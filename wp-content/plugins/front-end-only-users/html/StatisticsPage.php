...
<?php
	...
	if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Event_Location_Title', 'Count(Event_Value)', 'Event_Date' ) ) ) { $OrderBy = $_GET['OrderBy']; }
	else { $OrderBy = 'Count(Event_Value)'; }
	...
	if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Statistics") {$Sql .= "ORDER BY " . $OrderBy . " " . $Order . " ";}
	...
	<?php
		if ($myrows) {
	 		foreach ($myrows as $Link) {
				$Click_Count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(User_Event_ID) FROM $ewd_feup_user_events_table_name WHERE Event_Location=%s", $Link->Event_Location ) );
				$Last_Click = $wpdb->get_var( $wpdb->prepare( "SELECT Event_Date FROM $ewd_feup_user_events_table_name WHERE Event_Location=%s ORDER BY Event_Date DESC", $Link->Event_Location ) );
...
<?php
	...
	if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Event_Target_Title', 'Count(Event_Value)', 'Event_Type', 'Event_Date' ) ) ) { $OrderBy = $_GET['OrderBy']; }
	else { $OrderBy = 'Count(Event_Value)'; }
...
<table class="wp-list-table widefat tags sorttable fields-list ui-sortable" cellspacing="0">
	...
	<?php
		if ($myrows) {
	 		foreach ($myrows as $Link) {
				$Click_Count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(User_Event_ID) FROM $ewd_feup_user_events_table_name WHERE Event_Value=%s", $Link->Event_Value ) );
				$Last_Click = $wpdb->get_var( $wpdb->prepare( "SELECT Event_Date FROM $ewd_feup_user_events_table_name WHERE Event_Value=%s ORDER BY Event_Date DESC", $Link->Event_Value ) );
				echo "<tr id='User-" . $Link->Event_Value ."'>";
				echo "<td class='name column-name'>";
				echo "<strong>";
...
<?php
	...
	if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Username', 'User_Last_Login', 'User_Total_Logins', 'User_Date_Created' ) ) ) { $OrderBy = $_GET['OrderBy']; }
	else { $OrderBy = 'User_Last_Login'; }
	...
	$Number_of_Pages = ceil($num_rows/20);
	$Current_Page_With_Order_By = "admin.php?page=EWD-FEUP-options&DisplayPage=Statistics";
	if (isset($_GET['OrderBy'])) {$Current_Page_With_Order_By .= "&OrderBy=" . $OrderBy . "&Order=" . $_GET['Order'];}?>
