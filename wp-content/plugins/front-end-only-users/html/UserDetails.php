...
<!-- The details of a specific user for editing, based on the user ID -->
	<?php $UserDetails = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $ewd_feup_user_fields_table_name WHERE User_ID ='%d'", intval( $_GET['User_ID'] ) ) ); ?>
	<?php $UserAdmin = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $ewd_feup_user_table_name WHERE User_ID ='%d'", intval( $_GET['User_ID'] ) ) ); ?>
	<?php $Levels = $wpdb->get_results( "SELECT * FROM $ewd_feup_levels_table_name ORDER BY Level_Privilege ASC" ); ?>
	<?php 
		if (isset($_GET['Page'])) {$Page = intval( $_GET['Page'] );}
		else {$Page = 1;}

		if ( isset( $_GET['OrderBy'] ) and in_array( $_GET['OrderBy'], array( 'Event_Type', 'Event_Location_Title', 'Event_Target_Title', 'Event_Date' ) ) ) { $OrderBy = $_GET['OrderBy']; }
		...
		$myrows = $wpdb->get_results( $wpdb->prepare( $Sql, intval( $_GET['User_ID'] ) ) );
			...
			<div class="col-wrap ewd-feup-user-details-wrap">
				...
				<div class="form-wrap UserDetail">
					...
					<form id="addtag" method="post" action="admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_EditUser&DisplayPage=Users" class="validate" enctype="multipart/form-data">
						...
						<input type="hidden" name="User_ID" value="<?php echo esc_attr( intval( $_GET['User_ID'] ) ); ?>" />
									...
									<table class="form-table">
										...
										<?php foreach ($Fields as $Field) {
											...
											<tr>
												...
												<td>
													<?php if ($Field->Field_Type == "text") {?><input name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-regular-text' id="<?php echo $Field->Field_Name; ?>" type="text" value="<?php echo esc_attr( $Value );?>" size="60" />
													<?php } elseif ($Field->Field_Type == "mediumint") {?><input name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-regular-text' id="<?php echo $Field->Field_Name; ?>" type="text" value="<?php echo esc_attr( $Value );?>" size="60" />
													<?php } elseif ($Field->Field_Type == "email") {?><input name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-regular-text' id="<?php echo $Field->Field_Name; ?>" type="email" value="<?php echo esc_attr( $Value );?>" size="60" />
													<?php } elseif ($Field->Field_Type == "tel") {?><input name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-regular-text' id="<?php echo $Field->Field_Name; ?>" type="tel" value="<?php echo esc_attr( $Value );?>" size="60" />
													<?php } elseif ($Field->Field_Type == "url") {?><input name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-regular-text' id="<?php echo $Field->Field_Name; ?>" type="url" value="<?php echo esc_attr( $Value );?>" size="60" />
													<?php } elseif ($Field->Field_Type == "date") {?>
																<input name='<?php echo $Field->Field_Name; ?>' id='ewd-feup-register-input-<?php echo $Field->Field_ID; ?>' class='ewd-feup-date-input pure-input-1-3' type='date' value='<?php echo esc_attr( $Value );?>' />
													<?php } elseif ($Field->Field_Type == "datetime") { ?>
																<input name='<?php echo $Field->Field_Name; ?>' id='ewd-feup-register-input-<?php echo $Field->Field_ID; ?>' class='ewd-feup-datetime-input pure-input-1-3' type='datetime-local' value='<?php echo esc_attr( $Value );?>' />
													<?php } elseif ($Field->Field_Type == "textarea") { ?>
																<textarea name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-large-text' id="<?php echo $Field->Field_Name; ?>"><?php echo esc_html( $Value ) ?></textarea>
													...
													<?php } elseif ($Field->Field_Type == "picture") { ?>
																...
																<br />
																<img src='<?php echo esc_attr( content_url("/uploads/ewd-feup-user-uploads/") . $Value ); ?>' alt='<?php echo $Field->Field_Name ?>' class='ewd-feup-profile-picture' /><br />
													...
													<?php } elseif ($Field->Field_Type == "radio") { ?>
																<?php $Options = explode(",", $Field->Field_Options); ?>
																<?php foreach ($Options as $Option) { ?><input type='radio' name="<?php echo $Field->Field_Name; ?>" class='ewd-admin-small-input' value="<?php echo esc_attr( $Option ); ?>" <?php if ($Value == $Option) {echo "checked";} ?>><?php echo $Option ?><br/><?php } ?>
													<?php } elseif ($Field->Field_Type == "checkbox") { ?>
																<?php $Options = explode(",", $Field->Field_Options); ?>
																<?php $User_Checkbox = explode(",", $Value); ?>
																<?php foreach ($Options as $Option) { ?><input type="checkbox" class='ewd-admin-small-input' name="<?php echo $Field->Field_Name; ?>[]" value="<?php echo esc_attr( $Option ); ?>" <?php if (in_array($Option, $User_Checkbox)) {echo "checked";} ?>><?php echo $Option; ?></br><?php } ?>
													<?php } ?>
						...
						<div class="ewd-feup-admin-edit-product-right">
							...
							<div class="ewd-feup-dashboard-new-widget-box ewd-widget-box-full ewd-feup-admin-closeable-widget-box ewd-feup-admin-widget-box-start-closed" id="ewd-feup-admin-edit-user-custom-fields-widget-box">
								...
								<div class="ewd-feup-dashboard-new-widget-box-bottom">
									<a class='delete-tag feup-confirm-one-user button-secondary ewd-feup-admin-delete-all-products-button ewd-feup-admin-edit-product-delete-user-button' href='<?php echo add_query_arg( '_wp_nonce', wp_create_nonce( 'ewd-feup-admin' ), 'admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_DeleteUser&DisplayPage=Users&User_ID=' . $UserAdmin->User_ID ); ?>'>
