...
function User_Search($atts, $content = null) {
	...
	$ReturnString .= EWD_FEUP_Add_Modified_Styles();
				...
				$ReturnString .= "<table class='ewd-feup-user-search-results'>";
					...
					if (is_array($Users)) {
						foreach ($Users as $User) {
								...
								foreach ($display_fields_array as $display_field_value) {
									$ReturnString .= "<td>";
									if ($user_profile_page != "") {$ReturnString .= "<a href='" . esc_attr( $user_profile_page ) . "?User_ID=" . intval( $User['User_ID'] ) . "'>";}
		...
		if (isset($user_message['Message'])) {$ReturnString .= esc_html( $user_message['Message'] );}
		if (!isset($Time)){ $Time = ""; }
		...
		$ReturnString .= "<div class='feup-pure-control-group'>";
		$ReturnString .= "<label for='Submit'></label><input type='submit' class='ewd-feup-submit feup-pure-button feup-pure-button-primary' name='Search_Submit' value='" . esc_attr( $submit_text ) . "'>";
		$ReturnString .= "</div>";
		$ReturnString .= "</form>";
		$ReturnString .= "</div>";
