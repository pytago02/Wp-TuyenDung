...
<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
	...
	if ( isset( $resume_edit ) && $resume_edit ) {
		printf( '<p><strong>' . esc_html__( "Bạn đang chỉnh sửa một công việc hiện có. %s", 'wp-job-manager' ) . '</strong></p>', '<a href="?job_manager_form=submit-job&new=1&key=' . esc_attr( $resume_edit ) . '">' . esc_html__( 'Tạo một công việc mới', 'wp-job-manager' ) . '</a>' );
	}
	?>
	...
	<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
		...
		<?php if ( $company_fields ) : ?>
			<h2><?php esc_html_e( 'Chi tiết công ty', 'wp-job-manager' ); ?></h2>
		...
		<p>
			...
			if ( isset( $can_continue_later ) && $can_continue_later ) {
				echo '<input type="submit" name="save_draft" class="button secondary save_draft" value="' . esc_attr__( 'Lưu bản nháp', 'wp-job-manager' ) . '" formnovalidate />';
			}
			?>
