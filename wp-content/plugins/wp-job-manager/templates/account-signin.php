	...
	<fieldset class="fieldset-logged_in">
		...
		<div class="field account-sign-in">
			<?php
				...
				printf( wp_kses_post( __( 'Bạn hiện đang đăng nhập với tư cách<strong>%s</strong>.', 'wp-job-manager' ) ), esc_html( $user->user_login ) );
			?>
...
<?php else :
	...
	<fieldset class="fieldset-login_required">
		<label><?php esc_html_e( 'Có một tài khoản?', 'wp-job-manager' ); ?></label>
		<div class="field account-sign-in">
			...
			<?php if ( $registration_enabled ) : ?>
				...
				<?php printf( esc_html__( 'Nếu bạn không có tài khoản, bạn có thể tạo một tài khoản bên dưới bằng cách nhập địa chỉ email/tên người dùng.', 'wp-job-manager' ), $account_required ? '' : esc_html__( 'optionally', 'wp-job-manager' ) . ' ' ); ?>
				<?php if ( $use_standard_password_email ) : ?>
					<?php printf( esc_html__( 'Chi tiết tài khoản của bạn sẽ được xác nhận qua email.', 'wp-job-manager' ) ); ?>
				<?php endif; ?>

			<?php elseif ( $account_required ) : ?>

				<?php echo wp_kses_post( apply_filters( 'submit_job_form_login_required_message',  __( 'Bạn phải đăng nhập để tạo một danh sách mới.', 'wp-job-manager' ) ) ); ?>
