...
if ( ! is_user_logged_in() ) {
    echo '<p>' . esc_html__( 'Vui lòng đăng nhập để xem các bài tuyển dụng của bạn.', 'wp-job-manager' ) . '</p>';
    return; // Dừng thực thi nếu người dùng không đăng nhập
}

// Lấy ID người dùng đang đăng nhập
$current_user_id = get_current_user_id();

// Cập nhật WP_Query để lấy các bài đăng tuyển dụng của người dùng hiện tại
$args = array(
    'post_type' => 'job_listing', // Loại bài đăng của WP Job Manager
    'post_status' => 'publish',   // Chỉ lấy các bài đăng đã xuất bản
    'author' => $current_user_id, // Lọc bài đăng theo tác giả là người dùng hiện tại
    'posts_per_page' => -1        // Lấy tất cả các bài đăng
);

// Thực hiện truy vấn để lấy bài đăng của nhà tuyển dụng
			...
			<form method="GET" action="" class="jm-form">
				<div style="display: flex; gap: 12px;">
					<input type="search" name="search" class="jm-ui-input--search-icon"
						placeholder="<?php esc_attr_e( 'Tìm kiếm', 'wp-job-manager' ); ?>"
		...
		<div class="jm-dashboard__actions">
			<?php if ( job_manager_user_can_submit_job_listing() ) : ?>
				<a class="jm-ui-button"
					href="<?php echo esc_url( get_permalink( $submit_job_form_page_id ) ); ?>"><span><?php esc_html_e( 'Đăng tuyển', 'wp-job-manager' ); ?></span></a>
			<?php endif; ?>
		</div>
	</div>
				...
				<?php echo Notice::dialog(
					[
						'message' => $search_input
							...
							? sprintf( __( 'Không tìm thấy kết quả cho "%s".', 'wp-job-manager' ), $search_input )
		...
		<?php else : ?>
			<div class="jm-dashboard-header">
				...
				<div class="jm-dashboard-job-column jm-dashboard-job-column-label applicants">
					<?php esc_html_e( 'Ứng viên', 'wp-job-manager' ); ?>
				</div>
			</div>
			<div class="jm-dashboard-rows">
				<?php foreach ( $jobs as $job ) : ?>
					<div class="jm-dashboard-job">
						...
						<div class="jm-dashboard-job-column applicants">
							<a href="<?php echo esc_url(home_url('/danh-sach-ung-vien?job_id=' . $job->ID)); ?>" class="jm-dashboard-action jm-ui-button--link">Xem ứng viên</a>
						</div>
					</div>
				<?php endforeach; ?>
			</div>
