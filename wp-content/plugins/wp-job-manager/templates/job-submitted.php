	...
	case 'publish' :
		$job_submitted_content = '<div class="job-manager-message">' . wp_kses_post(
			sprintf(
				...
				__( '%1$s được liệt kê thành công.Để xem danh sách của bạn <a href="%2$s">Bấm vào đây</a>.', 'wp-job-manager' ),
	...
	case 'pending' :
		$job_submitted_content = '<div class="job-manager-message">' . wp_kses_post(
			sprintf(
				...
				esc_html__( '%s Gửi thành công.Danh sách của bạn sẽ được nhìn thấy sau khi được phê duyệt.', 'wp-job-manager' ),
				esc_html( $wp_post_types[\WP_Job_Manager_Post_Types::PT_LISTING]->labels->singular_name )
			)
		);
	...
	default :
		...
		$job_submitted_content = '<div class="job-manager-message">' . wp_kses_post(
			...
			// translators: %1$s is the job listing post type name.
				__( '%1$s Gửi thành công.', 'wp-job-manager' ),
				esc_html( $wp_post_types[\WP_Job_Manager_Post_Types::PT_LISTING]->labels->singular_name )
			)
		) . '</div>';
